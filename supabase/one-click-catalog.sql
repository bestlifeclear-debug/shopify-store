-- ============================================================================
-- ОДИН РАЗ в Supabase: SQL Editor → вставить весь файл → Run
-- Чинит чтение каталога для anon-ключа и добавляет 15 демо-товаров.
-- ============================================================================

-- 1) RLS: сайт использует anon key — нужна политика SELECT
alter table public.products enable row level security;

drop policy if exists "products_select_all" on public.products;
-- Явно anon + authenticated: иначе PostgREST (ключ из сайта) может не проходить SELECT
...
-- 2) На всякий случай — права на чтение для API (PostgREST)
grant usage on schema public to anon, authenticated;
grant select on public.products to anon, authenticated;

-- 3) Демо-товары (повторный запуск безопасен: on conflict do nothing)
